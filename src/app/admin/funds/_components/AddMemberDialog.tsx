...
export function AddMemberDialog({
  ...
  const selectedUser = value.user_uuid
    ? availableUsers.find((u) => u.uuid === value.user_uuid)
    : undefined;
            ...
            >
              <SelectTrigger id="member-user" className="h-10 w-full">
                <SelectValue placeholder="Seleccionar usuario...">
                  {selectedUser ? selectedUser.full_name || selectedUser.username : null}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            {selectedUser ? (
              <p className="text-xs text-muted-foreground">
                ...
                {selectedUser.phone_number ? (
                  <span className="font-mono text-foreground">{selectedUser.phone_number}</span>
                ) : (
                  <span className="text-amber-600 dark:text-amber-400">
                    sin número — defínelo en Usuarios para la detección vía socio
                  </span>
                )}
              </p>
            ) : null}
